# SalaryLens (V2 - fhevm-solidity 0.9.x)

**Author:** Zama Bounty Program - Season 11 Submission

> A privacy-preserving salary aggregation contract using Fully Homomorphic Encryption (FHE)


## Overview

This contract demonstrates encrypted aggregation by calculating average salary without
...
This value is never directly readable - only used in encrypted computations

Kept as plaintext for gas efficiency and to enable division

Prevents double-submission to ensure accurate averaging

Populated after proof verification

Used for proof verification

Maps handle to whether it's been used

Inherits ZamaEthereumConfig which sets up the coprocessor addresses

This function:

**Parameters:**
- `submitter`: The address that submitted the salary
- `newCount`: The updated total count of submissions
- `requester`: The address that originally requested the average
- `handle`: The ciphertext handle for off-chain decryption
- `average`: The decrypted average salary value
- `encryptedSalary`: The encrypted salary value (externalEuint32 type)
- `inputProof`: Zero-knowledge proof validating the encrypted input

### `requestAverageDecryption()`

Request decryption of the average salary

This function:

**Returns:** handle The ciphertext handle for off-chain decryption

### `verifyDecryption()`

Verify the decryption proof and store the result

Called after off-chain relayer decrypts the value

**Parameters:**
- `abiEncodedCleartexts`: ABI-encoded cleartext values from relayer
- `decryptionProof`: Proof from Zama KMS via relayer-sdk

### `getCount()`

Get the current count of submitted salaries

This is a simple getter for the count state variable

**Returns:** The number of salaries that have been submitted

### `hasUserSubmitted()`

Check if an address has already submitted a salary

**Parameters:**
- `user`: The address to check

**Returns:** True if the user has submitted, false otherwise

### `getLastAverage()`

Get the last decrypted average for a specific address

Returns 0 if the address has never verified a decryption

**Parameters:**
- `user`: The address to query

**Returns:** The last decrypted average salary for this user

### `getPendingHandle()`

Get pending decryption handle for a user

**Parameters:**
- `user`: The address to query

**Returns:** The pending handle, or bytes32(0) if none
