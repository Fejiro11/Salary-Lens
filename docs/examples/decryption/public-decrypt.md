...
> Demonstrates the public decryption pattern with proof verification
...
Shows the complete 3-step decryption flow:
...
Marks the encrypted value as publicly decryptable

### `verifyDecryption()`

Step 3: Verify decryption proof and store result

The relayer-sdk provides both parameters:

**Parameters:**
- `abiEncodedCleartexts`: ABI-encoded decrypted value from relayer
- `decryptionProof`: KMS signatures proving correct decryption
